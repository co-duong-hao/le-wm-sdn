...
surprise score only reaches F1 $=0.0841$. A minimal PyTorch temporal GNN JEPA
implementation sanity check decreases validation loss but is not yet used as a
benchmark claim. These results position LeWM-SDN as a promising normal-only
predictive detector in controlled SDN dynamics, while also identifying
public-data calibration and neural tuning as unresolved limitations.
...
sanity check, while NumPy experiments provide faster baseline comparison,
figures, and report generation.
...
\begin{enumerate}
  ...
  architecture trained for a minimal implementation sanity check on normal
  synthetic windows.
\end{enumerate}
...
scores and the minimal PyTorch JEPA sanity check are not competitive.}
...
normal-only surprise scores are low; the PyTorch JEPA sanity check is zero.}
...
variants have poor recall. The minimal PyTorch JEPA public sanity check trains
...
is currently inadequate. This sanity check is included to document
executability, not to claim a tuned neural benchmark. The public anomaly result
is negative for the current formulation but valuable: it prevents the paper
from relying only on an easy generated dataset.
...
The PyTorch temporal GNN JEPA is reported only as an implementation sanity
check. Training loss decreases from $4.8049$ to $3.7158$ and validation loss
decreases from $3.7971$ to $3.1776$ in the recorded CPU run, which verifies
that the temporal graph encoder, JEPA predictor, and anomaly evaluator execute
end to end. Its anomaly F1 is only $0.0062$ under the default 99th-percentile
normal threshold, so it is not used as evidence of neural detection quality.
...
reference. The PyTorch temporal GNN JEPA is included as an implementation
sanity check, not as a tuned benchmark. This validates executability, not final
neural performance.
...
a temporally split public SDN trace, tune it beyond the current sanity check,
