...
and a more explicit method description. On the generated 1,200-timestep graph
dataset, the NumPy LeWM-SDN reference detector reaches F1 $=0.9796$, while
supervised logistic regression and a shallow MLP reach F1 $=1.0000$. On the
public NF-UNSW-NB15 graph test split, supervised logistic regression and NumPy
MLP reach F1 $=0.9908$, while the current LeWM-SDN latent surprise score only
reaches F1 $=0.0841$. Small 8-epoch PyTorch temporal GNN JEPA runs decrease
validation loss on both synthetic and public graphs, but the public run obtains
F1 $=0$ under the default normal-score threshold. These results should be
interpreted as honest prototype validation rather than a claim of superiority
over ML/DL baselines.
...
full public-benchmark tuning. The third contribution is executed on both the
synthetic graph and the public NF-UNSW-NB15 graph, although the public result
is unfavorable to the current anomaly score. The fourth contribution is now an
executed reproducibility path for NF-UNSW-NB15 and a prepared path for InSDN,
UNSW-NB15 CSV, CICDDoS, TON-IoT, and Bot-IoT.
...
this synthetic dataset. This is an important correction to the earlier draft: the current
evidence demonstrates that the predictive latent pipeline runs and is
competitive with unsupervised feature-distance detection, not that it is
superior to supervised ML/DL on a public benchmark. The perfect supervised
...
works well. This supports the claim that latent representation and phase-aware
scoring are useful even in the controlled setting.

\section{Discussion}

The revised experiment directly addresses the absence of baselines. It also
shows why the synthetic dataset cannot be the final benchmark: supervised
classifiers solve it perfectly. A publishable Q4-level paper should therefore
run the PyTorch temporal GNN JEPA on the public NF-UNSW-NB15 graph or on InSDN
and compare against classical ML and DL baselines. The current public benchmark
already includes logistic regression, a lightweight gradient-boosted stump
baseline, MLP, one-class distance, and next-state surprise baselines. The
synthetic benchmark also includes PyTorch CNN and LSTM baselines. The next
public benchmark iteration should add external packages such as XGBoost or
...
requires public data and tuning before it can support a publication claim.
...
conversion scripts. The current executable result remains an offline controlled
validation rather than a final public benchmark. The next decisive step is to
run the implemented temporal GNN JEPA on the public NF-UNSW-NB15 graph or
InSDN, tune it beyond the current CPU smoke run, and compare it with the
