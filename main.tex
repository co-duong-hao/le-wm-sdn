...
\providecommand{\Description}[1]{\par\small\emph{Alt text:} #1}
...
surprise score reaches only F1 $=0.0841$. After extending the neural training
to 100 CPU epochs and selecting the anomaly threshold on a held-out calibration
split, the PyTorch temporal GNN JEPA reaches test F1 $=0.8718$ and balanced
accuracy $=0.7045$ on NF-UNSW-NB15. These results position LeWM-SDN as a
promising normal-only predictive detector in controlled SDN dynamics and show
that public-data threshold calibration is essential.
...
latent phase drift.}
\Description{A left-to-right architecture diagram showing SDN telemetry
converted into dynamic graph snapshots, encoded into latent progress and
content states, predicted forward by JEPA, and scored for DDoS anomaly
detection.}
...
defects. The calibrated public neural result is useful, although it still
trails the supervised baselines.
...
for a PyTorch environment. The recorded implementation sanity check uses a
smaller CPU configuration to verify the code path on the current machine; it is
not reported as a tuned model benchmark.
...
The repository also exposes latent rollout and defense-cost functions intended
for future mitigation planning. They are not part of the evaluated detection
method in this paper because the available public intrusion datasets do not
contain logged mitigation actions or post-action network states.
...
The current machine has NumPy, PIL, MiKTeX, and a local PyTorch virtual
environment. The PyTorch training path is executed as a minimal implementation
sanity check on the synthetic dataset and as a longer 100-epoch calibrated run
on the public NF-UNSW-NB15 graph. NumPy experiments provide faster baseline
...
normal prefix.}
\Description{A horizontal timeline of the generated SDN dataset showing normal
traffic at the start and between repeated DDoS burst intervals.}
\label{fig:timeline}
\end{figure}
...
\begin{enumerate}
  ...
  architecture trained on normal windows. The synthetic run is a minimal sanity
  check; the public NF-UNSW-NB15 run is trained for 100 CPU epochs and evaluated
...
For the public PyTorch JEPA run, the initial short CPU configuration was
replaced by a 100-epoch run with batch size 128, learning rate $2\cdot10^{-4}$,
weight decay $5\cdot10^{-4}$, and the same compact 32-dimensional latent
configuration. A 200-epoch run was also tested, but its calibrated held-out F1
was lower than the 100-epoch checkpoint, so the 100-epoch checkpoint is used in
the reported public neural result.
...
same synthetic dataset and is reported only as an implementation sanity check
...
For the PyTorch JEPA score, evaluation additionally tests threshold strategies:
normal-score quantiles 90/95/97.5/99, normal mean plus $k$ standard deviations
for $k\in\{1,2,3\}$, and best-F1 threshold selection on a calibration split.
The chosen threshold is then applied to the held-out test split.
...
\Description{A bar chart comparing F1 scores on the synthetic SDN/DDoS dataset.
Supervised classifiers are near one, one-class distance and LeWM-SDN are high,
raw-feature ridge surprise is low, and the minimal PyTorch JEPA sanity check is
near zero.}
\label{fig:baseline}
\end{figure}
...
windows.}
\Description{A two-group score comparison showing normal windows with low mean
anomaly scores and DDoS windows with much higher predictive latent surprise.}
\label{fig:score}
\end{figure}
...
Supervised baselines perform well. The calibrated 100-epoch PyTorch temporal
GNN JEPA improves substantially over the default threshold result, but still
trails the supervised classifiers.}
\Description{A bar chart comparing held-out F1 scores on NF-UNSW-NB15. The
supervised logistic, MLP, CNN, and LSTM baselines are high; one-class and
normal-only reference scores are low; the calibrated PyTorch temporal GNN JEPA
is below the supervised classifiers but much higher than the uncalibrated
...
well on NF-UNSW-NB15, while the current one-class and NumPy predictive-surprise
variants have poor recall. The PyTorch JEPA result changes substantially after
training and calibration. With the default 99th-percentile normal threshold,
the 100-epoch model still gives F1 $=0.0006$, because only 52 transitions are
flagged. However, the score distribution is meaningful: attack transitions
have higher mean score than normal transitions ($0.4622$ versus $0.3271$), and
the test AUC is $0.7199$. Selecting the threshold on a held-out calibration
split gives test precision $=0.7731$, recall $=0.9994$, F1 $=0.8718$,
accuracy $=0.8037$, and balanced accuracy $=0.7045$. This result does not beat
the supervised classifiers, but it removes the earlier F1 $=0$ failure and
shows that threshold calibration is a central part of the neural detector.
...
The synthetic PyTorch temporal GNN JEPA remains an implementation sanity check.
On the public graph, however, the 100-epoch model gives a useful neural result
once threshold calibration is applied. Training for 200 epochs was tested as a
simple convergence check; it lowered validation loss slightly but reduced
calibrated test F1 from $0.8718$ to $0.8311$, so the 100-epoch checkpoint is
reported.
...
label-supervised classification. The positive evidence is now twofold:
LeWM-SDN detects controlled SDN/DDoS disruptions without attack labels and
improves substantially over raw-feature predictive surprise, and the trained
Temporal GNN JEPA reaches useful public-data F1 after calibration. At the same
time, the synthetic dataset cannot be the final benchmark because supervised
classifiers solve it nearly perfectly. A stronger public normal-only evaluation
should still be added, preferably on InSDN or a temporally split SDN trace. The
current public benchmark already includes logistic
...
beats supervised classifiers. The implemented PyTorch training path now runs
for a longer public-data experiment, and threshold calibration lifts the public
neural result from near-zero F1 to a useful but still non-supervised level.

\subsection{Future Mitigation Planning}

The codebase includes a design hook for model-predictive mitigation, but this
paper deliberately excludes it from the evaluated method. Public datasets used
here provide traffic records and labels, not interventions such as drop-flow,
rate-limit, reroute, or filter-install actions followed by observed
post-action network states. A future controlled SDN testbed can use the learned
latent dynamics for MPC/CEM-style action search. Given candidate action
sequences $u_{t:t+H-1}^{(s)}$, the model would roll out latent futures
$\hat{z}_{t+1:t+H}^{(s)}$ and evaluate:
\begin{equation}
C^{(s)} =
\sum_{j=1}^{H}
\left[
...
\right].
\end{equation}
Only the first action of the selected sequence would be executed before
observing the next network state and replanning. This remains future work, not
an experimental contribution of the current paper.
...
The strongest controlled synthetic result still comes from the PCA--ridge NumPy
reference. The PyTorch temporal GNN JEPA now has a stronger public-data result
after 100 epochs and calibration, but it remains weaker than supervised
classifiers and needs validation on SDN-native InSDN.
...
distance on controlled SDN graph dynamics. On public NF-UNSW-NB15, a 100-epoch
Temporal GNN JEPA with calibration reaches F1 $=0.8718$, which is much stronger
than the previous default-threshold failure but still below supervised
classifiers. The next decisive step is to run the implemented temporal GNN JEPA
on InSDN or a temporally split public SDN trace and compare it with the
existing boosting/CNN/LSTM/MLP baselines plus external random-forest, XGBoost,
