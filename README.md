...
Branch này bổ sung pipeline nghiên cứu ban đầu để thử chuyển ý tưởng LeWM sang
bài toán phát hiện DDoS trên dữ liệu network flow. Đường chạy LeWM gốc cho bài
toán điều khiển từ pixel vẫn được giữ nguyên; phần demo dưới đây chỉ dùng các
script trong `tools/`.

### Mục tiêu demo

Demo hiện tại chứng minh được các bước sau:
...
thống. Đây là mốc nghiên cứu V0/V1 để kiểm tra pipeline và tín hiệu
`prediction surprise`.
...
Không đặt dataset vào git. Repo đã ignore `datasets/`, `data/`, `outputs/`,
`runs/`, `checkpoints/`, `*.pt`, `*.pth`, `*.ckpt`.
