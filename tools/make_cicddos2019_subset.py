"""Create a small CICDDoS2019 CSV subset for early experiments.

The full CICDDoS2019 CSV dump is very large and highly imbalanced. This script
streams the CSVs in chunks, keeps numeric flow features, normalizes column names,
and writes a bounded sample per label.
"""
...
def sample_group(group: pd.DataFrame, remaining: int, seed: int) -> pd.DataFrame:
    if remaining <= 0 or group.empty:
        return group.iloc[0:0]
    take = min(remaining, len(group))
    return group.sample(n=take, random_state=seed) if take < len(group) else group
    ...
    for csv_path in files:
        ...
        for chunk in pd.read_csv(csv_path, chunksize=args.chunksize, low_memory=False):
            ...
            for label, group in cleaned.groupby("label", sort=False):
                ...
                picked = sample_group(group, remaining, args.seed + chunk_index)
                if picked.empty:
                    continue
            ...
            if written and all(count >= args.max_per_label for count in written.values()):
                pass
